// The actual Security Alerts detection logic, with no "server-only" guard
// and no dependency on the app's db singleton (lib/db.ts) — both of those
// throw when imported outside a Next.js request, which one-off scripts
// (e.g. scripts/hipaa-compliance-check.ts, run as a standalone ECS task)
// need to avoid. Takes a Prisma client as a parameter instead, so
// lib/data/security-alerts.ts's getSecurityAlerts() and any standalone
// script can share this one implementation without either drifting from
// the other.
import type { PrismaClient } from "@/app/generated/prisma/client";

// Only the two delegates this module actually calls — accepting the full
// PrismaClient (or lib/db.ts's soft-delete-extended Db, which isn't
// structurally identical to it) would force every caller onto one exact
// client type. Neither User nor AuditLog is soft-delete-extended, so a
// plain PrismaClient (e.g. scripts/hipaa-compliance-check.ts's standalone
// instance) and the app's extended db singleton are both assignable here.
type SecurityAlertsDb = Pick<PrismaClient, "user" | "auditLog">;
...
export async function computeSecurityAlerts(clinicId: string, db: SecurityAlertsDb) {
