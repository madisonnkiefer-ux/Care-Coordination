                  ...
                  <li key={v.id} className="py-3 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge color={v.spokeWithMember === true ? "green" : v.spokeWithMember === false ? "red" : "slate"}>
                          {v.spokeWithMember === true ? "Spoke with member" : v.spokeWithMember === false ? "Did not speak with member" : "Not noted"}
                        </Badge>
                        <span className="text-xs text-stone-500">{formatDate(v.visitedAt)}</span>
                      </div>
                    </div>
                    <p className="mt-1 text-xs text-stone-500">Logged by {v.coordinator.name} · {formatDateTime(v.createdAt)}</p>
        ...
        <div className="space-y-6">
          <Card title="Log a Home Visit">
            <form action={logHomeVisit} className="space-y-3">
              ...
              <div>
                <p className="mb-1 text-xs font-medium uppercase tracking-wide text-stone-500">Spoke with member?</p>
                <div className="flex gap-6">
                  <label className="flex items-center gap-2 text-sm text-stone-600">
                    <input type="radio" name="spokeWithMember" value="yes" className="h-4 w-4" />
                    Yes
                  </label>
                  <label className="flex items-center gap-2 text-sm text-stone-600">
                    <input type="radio" name="spokeWithMember" value="no" className="h-4 w-4" />
                    No
                  </label>
                </div>
              </div>
