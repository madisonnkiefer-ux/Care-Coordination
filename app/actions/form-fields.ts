...
export async function updateFormFieldOverride(fieldKey: string, formData: FormData) {
  ...
  if (required) {
    ...
    if (missing.length > 0) {
      throw new Error(
        `This field drives required-assessment logic — these values can't be removed: ${missing.join(", ")}`
      );
    }
  }

  const labelRaw = formData.get("label");
  const label = def.labelEditable && typeof labelRaw === "string" && labelRaw.trim() !== "" ? labelRaw.trim() : null;

  await db.formFieldOverride.upsert({
    where: { clinicId_fieldKey: { clinicId: session.clinicId, fieldKey } },
    create: { clinicId: session.clinicId, fieldKey, label, options, updatedById: session.userId },
    update: { label, options, updatedById: session.userId },
  });
