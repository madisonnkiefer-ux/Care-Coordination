...
export function HraTab({ memberId, data }: { memberId: string; data: NonNullable<Awaited<ReturnType<typeof getHraFormData>>> }) {
  const { tools, latestCompletedDate } = data;

  return (
    <form action={saveHra.bind(null, memberId)} className="space-y-6 p-8">
            ...
            <div key={tool.key} className="rounded-lg border border-slate-200 p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <label className="flex items-center gap-2 text-xs text-slate-600">
                  <input type="checkbox" name={`completed_${tool.key}`} defaultChecked={Boolean(tool.existing?.completedAt)} />
                  Completed
                </label>
              </div>
