...
import type { getCnaFormData } from "@/lib/data/cna";

export function CnaTab({ memberId, data }: { memberId: string; data: NonNullable<Awaited<ReturnType<typeof getCnaFormData>>> }) {
  const { draft, latestCompletedDate, domains } = data;

  return (
    <form action={saveCna.bind(null, memberId)} className="space-y-6 p-8">
      ...
      <Card title="1. Assessment Details">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-slate-500">Assessment Type</label>
            <select
              name="assessmentType"
            ...
            <div key={domain.key} className="rounded-lg border border-slate-200 p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <label className="flex items-center gap-2 text-xs text-slate-600">
                  <input type="checkbox" name={`hasNeeds_${domain.key}`} defaultChecked={domain.existing?.hasNeeds ?? false} />
                  Needs identified
                </label>
              </div>
